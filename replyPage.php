	...
	<body>
		...
		<div class="container">
			<div class="row row-content">
				...
				<form method="post" action="replyToContactUs.php">
                    <?php
                        $id = $_POST['reply'];
                        $id = substr($id, 5);
                        $_SESSION['uerid'] = $id;
                        $conn = mysqli_connect("localhost","root","password","chat");
                        $stm = "select name from contactus where id = ".$id.";";
                        $res = mysqli_query($conn,$stm);
                        if(mysqli_num_rows($res) > 0)
                        {
                            $row = mysqli_fetch_assoc($res);
                            echo "<label> Reply to ".$row['name']."</label><br>";
                        }
                    ?>
                    <textarea maxlength="500" rows="5" name="replyText"></textarea><br><br>
